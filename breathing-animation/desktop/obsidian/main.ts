...
export default class MindfulBreathingPlugin extends Plugin {
  async onload() {
    this.registerView(VIEW_TYPE_BREATHING, (leaf: WorkspaceLeaf) => new BreathingView(leaf));
...
class BreathingView extends ItemView {
  ...
  async onOpen() {
    ...
    visualizer.style.transition = 'transform 4s cubic-bezier(0.37, 0, 0.63, 1), background-color 4s cubic-bezier(0.37, 0, 0.63, 1)';
  ...
  startAnimation(el: HTMLElement, textEl: HTMLElement, techniqueKey: string) {
    ...
    const run = () => {
      ...
      if (p.name.toLowerCase().includes('hold')) {
        el.style.transition = `background-color ${p.duration}ms cubic-bezier(0.37, 0, 0.63, 1)`;
      } else {
        el.style.transition = `transform ${p.duration}ms cubic-bezier(0.37, 0, 0.63, 1), background-color ${p.duration}ms cubic-bezier(0.37, 0, 0.63, 1)`;
      }
