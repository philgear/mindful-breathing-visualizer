...
const audio_class = `class AudioController {
    constructor() {
...
const frameworks = [
    'react/BreathingVisualizer.jsx',
    'vue/BreathingVisualizer.vue',
    'svelte/BreathingVisualizer.svelte',
    'astro/BreathingVisualizer.astro'
];

frameworks.forEach(fw => {
    const filePath = path.join(__dirname, 'frontend', fw);
    let content = fs.readFileSync(filePath, 'utf8');

    if (fw.includes('react/') || fw.includes('astro/')) {
        content = content.replace(/class AudioController \{[\s\S]*?\n\}/m, audio_class);
    } else {
        content = content.replace(/const audioController = \{[\s\S]*?\n\};\n/m, audio_class + '\\nconst audioController = new AudioController();\\n');
    }
